...
function Login() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // err is the rejectWithValue payload: { message: "Invalid credentials" }
        ...
        <div className="bg-base-100 rounded-2xl shadow-xl border border-base-300 w-full max-w-md overflow-hidden">
          <div className="p-8">
            ...
            <div className="mb-7">
              ...
              <p className="text-sm text-base-content/60 mt-1">Sign in to continue to CodeArena</p>
            </div>

            {/* Error Banner — only shown after a submission attempt */}
            {(localError || error) && (
              <div className="alert alert-error text-sm py-3 mb-5 shadow-md">
                <span>{typeof (localError || error) === 'string' ? (localError || error) : (localError || error)?.message || 'Invalid credentials. Please try again.'}</span>
              </div>
            )}
              ...
              <div className="flex items-center justify-between text-sm py-1">
                <label className="flex items-center gap-2 cursor-pointer select-none">
                  <input
                    ...
                    {...register('rememberMe')}
                  />
                  <span className="text-xs font-medium text-base-content/75">Remember me</span>
                </label>
                <NavLink
                  to="/forgot-password"
